...
def add_url():
	# db = mongo.db
	url_maping = mongo.db.record_table
	url_maping.insert({ "key":"abcd", "original_url":"google.com", "custom":True, "visits": 10})
	return "New URL Added in table"

@app.route("/find_url")
def find_url():
	url_collection = mongo.db.record_table
...
def custom_url_visits(custom_url):
	response = mongo.record_table.find_one({'custom_url':custom_url})
	if response:
		return " and ".join([response['original_url'], response['custom_url'], str(response['visits'])])
		# mongo.record_table.update({'custom_url': response['custom_url']}, {'visits': int(response['visits']+1)})
	else:
